...
生成 handoff 前，先检查与当前任务直接相关的代码、配置、Git diff、实际验证结果和当前事实型文档是否一致。

- 当前事实型文档已被本次实现改变时，先通过 `aki-context-sync` 更新权威文档，再生成 handoff。
- 候选设计、Roadmap、未来计划与当前实现不同，不自动视为文档过期。
- 无法确认代码还是文档代表正确当前状态时，不擅自选择，在 handoff 中记录为冲突或未确认。
- 只检查当前任务直接相关内容，不借 handoff 扩大成全项目文档审计。

handoff 优先引用已经收敛后的权威文档，不在交接文件中维护第二套“当前事实”。
...
- **目标**：当前任务最终目标；
- **当前状态**：已完成、未完成和仍未验证的部分；
- **约束**：下一位 Agent 不知道就可能做错事的当前约束；
- **相关产物**：关键文件、文档、commit、branch、PR 或其他证据；
- **Git 状态**：当前 branch、HEAD、working tree 状态；
- **验证**：明确区分已验证和未验证；
- **阻塞 / 未知项**：真正阻碍继续推进的问题；
- **避免重复**：已经确认失败且重复尝试会明显浪费时间或产生风险的方案；
- **下一步**：一个具体、低歧义的建议起点。
...
接管 handoff 时，不直接相信其中的状态或执行“下一步”。

1. 读取当前项目 `AGENTS.md` 和任务需要的中央原则。
2. 检查当前 branch、HEAD 和 working tree。
3. 对比 handoff 记录的 Git 状态。
4. 读取 handoff 指向的关键产物和权威文档。
5. 重新核验会影响当前行动的重要声明。
...
发生冲突时，以当前用户指令、代码、配置、Git 状态、项目权威文档和实际验证结果为证据重新判断。

不要机械规定“代码永远覆盖文档”或“文档永远覆盖代码”；先判断文档表达的是当前事实、未来计划还是候选设计。

handoff 只是上一 Agent 的交接声明。

### 交接与授权

“下一步”、TODO、推荐操作以及“建议 merge / 发布 / 删除”等内容都不是用户授权。
